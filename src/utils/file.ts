...
const temporaryFilenamePattern =
  /^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}_[A-Za-z0-9._-]+$/i;

const resolveTemporaryFilePath = (temporaryUrl: string): string => {
  ...
  if (
    ...
    throw new BadRequestError("Invalid temporary file URL.");
  }

  const temporaryDirectory = isProd ? "/tmp" : path.join(process.cwd(), "tmp");
  return path.join(temporaryDirectory, filename);
};

const requireStorageConfiguration = () => {
  if (!bucketName || !region) {
    throw new ConfigurationError("S3 storage is not configured");
  }
};
 ...
 * - s3://bucket/key URLs
 */
export function extractS3Key(filePath: string): string | null {
...
const resolveS3Key = (fileKeyOrUrl: string): string => {
  const extracted = extractS3Key(fileKeyOrUrl);
  if (extracted) return extracted;
  if (!fileKeyOrUrl.includes("://")) return fileKeyOrUrl.replace(/^\/+/, "");
  throw new Error("The stored file does not belong to the configured bucket");
};

export const createPresignedFileUrl = async (
  fileKeyOrUrl: string,
  filename: string,
...
export const uploadFile = async (
  tmpFilePath: string,
  removeTemporaryAfterUpload = true,
): Promise<string> => {
  const realFilePath = resolveTemporaryFilePath(tmpFilePath);

  if (!existsSync(realFilePath)) {
    throw new BadRequestError("Temporary file does not exist or has expired.");
  }
  ...
  if (removeTemporaryAfterUpload && existsSync(realFilePath)) {
...
export const removeTemporaryFile = (temporaryUrl: string): boolean => {
  const realFilePath = resolveTemporaryFilePath(temporaryUrl);
  if (!existsSync(realFilePath)) return false;
  unlinkSync(realFilePath);
  return true;
};
