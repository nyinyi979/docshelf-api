...
exports.removeFile = exports.removeFiles = exports.uploadFileRealPath = exports.removeTemporaryFile = exports.uploadFile = exports.duplicateFileS3 = exports.saveFileToTmp = exports.uploadFileByBuffer = exports.createPresignedFileUrl = void 0;
...
const temporaryFilenamePattern = /^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}_[A-Za-z0-9._-]+$/i;
const resolveTemporaryFilePath = (temporaryUrl) => {
    const normalized = temporaryUrl.replace(/\\/g, "/");
    const filename = normalized.slice("/tmp/".length);
    if (!normalized.startsWith("/tmp/") ||
        filename.includes("/") ||
        !temporaryFilenamePattern.test(filename)) {
        throw new errors_1.BadRequestError("Invalid temporary file URL.");
    }
    const temporaryDirectory = isProd ? "/tmp" : node_path_1.default.join(process.cwd(), "tmp");
    return node_path_1.default.join(temporaryDirectory, filename);
};
const requireStorageConfiguration = () => {
    if (!bucketName || !region) {
        throw new errors_1.ConfigurationError("S3 storage is not configured");
    }
};
/**
...
const resolveS3Key = (fileKeyOrUrl) => {
    const extracted = extractS3Key(fileKeyOrUrl);
    if (extracted)
        return extracted;
    if (!fileKeyOrUrl.includes("://"))
        return fileKeyOrUrl.replace(/^\/+/, "");
    throw new Error("The stored file does not belong to the configured bucket");
};
const createPresignedFileUrl = async (fileKeyOrUrl, filename, expiresInSeconds = 2 * 60 * 60) => {
    requireStorageConfiguration();
    const key = resolveS3Key(fileKeyOrUrl);
    const url = await (0, s3_request_presigner_1.getSignedUrl)(aws_1.default, new client_s3_1.GetObjectCommand({
        Bucket: bucketName,
        Key: key,
...
exports.createPresignedFileUrl = createPresignedFileUrl;
 ...
 */
const uploadFile = async (tmpFilePath, removeTemporaryAfterUpload = true) => {
    const realFilePath = resolveTemporaryFilePath(tmpFilePath);
    if (!(0, node_fs_1.existsSync)(realFilePath)) {
        throw new errors_1.BadRequestError("Temporary file does not exist or has expired.");
    }
    ...
    if (removeTemporaryAfterUpload && (0, node_fs_1.existsSync)(realFilePath)) {
        (0, node_fs_1.unlinkSync)(realFilePath);
    }
    return uploadedUrl;
};
exports.uploadFile = uploadFile;
const removeTemporaryFile = (temporaryUrl) => {
    const realFilePath = resolveTemporaryFilePath(temporaryUrl);
    if (!(0, node_fs_1.existsSync)(realFilePath))
        return false;
    (0, node_fs_1.unlinkSync)(realFilePath);
    return true;
};
exports.removeTemporaryFile = removeTemporaryFile;
/**
